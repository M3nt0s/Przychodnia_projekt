...
///Funkcja odpowiadajaca za przydzial pamieci oraz wprowadzanie danych, opartych na strukturze 'pacjenci'; malloc() alokuje pamiec wielkosci struktury 'pacjenci' i zwraca wskaznik na poczatek zaalokowanego bloku pamieci.
int nowy_pacjent(int id){

    struct pacjenci *dodaj_wpis = (struct pacjenci *)malloc(sizeof(struct pacjenci));
...
///Funkcja odpowiadajaca za wyswietlenie calej listy pacjentow; jesli 'poczatek' rozni sie od NULL, wtedy wartosc wskaznika 'tmp' przyjmuje wartosc wskaznika 'poczatek'; jesli 'tmp' nie jest NULL-em, to funkcja - dzieki petli while() i przechodzeniu do nastepnych elementow listy - wyswietla wszystkie elementy listy dwukierunkowej po kolei.
...
///Funkcja odpowiedzialna za wczytanie pacjentow z pliku "Baza_Przychodnia_Pacjenci.txt"; najpierw plik otwierany jest tylko do odczytu.
...
///Funkcja zapisujaca pacjentow do pliku; plik otwieramy tylko do zapisu; do wskaznika 'dodaj_wpis' przypisujemy wartosc wskaznika wskazujacego na 'poczatek' listy; po jej elementach poruszamy sie dzieki petli while() i przypisywaniu wskaznikowi 'dodaj_wpis' wartosci wskaznika 'nastepny'.
...
/// Funkcja odpowiadajaca za usuwanie konkretnego pacjenta - parametrem wywolania jest tutaj ID pacjenta.
...
/// Funkcja umozliwiajaca edycje pacjentow wedlug podanego ID; funkcja pozwala na wprowadzenie kazdej nowej danej z osobna (ID, imie, nazwisko etc).
...
///Funkcja pozwalajaca na wyszukiwanie pacjenta wg ID, imienia, nazwiska, numeru PESEL lub Oddzialu NFZ.
int menu_wyszukiwania_pacjentow(){
    ...
    int id_pacjenta;
    char imie_pacjenta[ROZMIAR];
    char nazwisko_pacjenta[ROZMIAR];
    char pesel_pacjenta[ROZMIAR];
    char oddzial_pacjenta[ROZMIAR];
...
///Ponizsza funkcja pozwala na wyswietlenie wszystkich szcegolowych danych wybranego pacjenta.
...
/// Funkcja zamieniajaca dane dwoch pacjentow, przekazanych przez argument funkcji jako wskazniki na odpowiednie struktury.
...
/// Funkcja odpowiedzialna za dzialanie menu sortowania pacjentow - sortowanie rosnaco i malejaco, wg odpowiednich pol (imie, nazwisko, PESEL etc).
...
/// Funkcja sortujaca pacjentow malejaco.
